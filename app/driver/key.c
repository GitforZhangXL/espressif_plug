...
key_init(struct keys_param *keys)
{
    ...
    for (i = 0; i < keys->key_num; i++) {
        ...
        pGPIOConfig->GPIO_IntrType = GPIO_PIN_INTR_NEGEDGE;//下降沿触发
        pGPIOConfig->GPIO_Pullup = GPIO_PullUp_EN;//上拉使能，上拉到高电平
...
key_5s_cb(struct single_key_param *single_key)
{
   ...
   //check this gpio pin state
    if (0 == GPIO_INPUT_GET(GPIO_ID_PIN(single_key->gpio_id))) {
    	...
    	gpio_pin_intr_state_set(GPIO_ID_PIN(single_key->gpio_id), GPIO_PIN_INTR_NEGEDGE);//下降沿触发
