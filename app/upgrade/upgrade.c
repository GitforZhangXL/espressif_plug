...
#define UPGRADE_DATA_SEG_LEN 512

#define UPGRADE_RETRY_TIMES 60
...
void upgrade_task(void *pvParameters)
{
    ...
    while (retry_count++ < UPGRADE_RETRY_TIMES) {
        ...
        server->sockaddrin.sin_addr.s_addr= inet_addr(TEST_SERVER);
#endif
        //server->sockaddrin.sin_addr.s_addr= inet_addr("115.29.202.58");
	    if(0 != connect(sta_socket,(struct sockaddr *)(&server->sockaddrin),sizeof(struct sockaddr)))
	    {
	        close(sta_socket);
	        printf("connect fail!\r\n");
...
finish:
	...
	if(upgrade_crc_check(system_get_fw_start_sec(),sumlength) != 1)
	{
		printf("upgrade crc check failed !\n");
		server->upgrade_flag = false;
        system_upgrade_flag_set(UPGRADE_FLAG_IDLE);	
...
system_upgrade_start(struct upgrade_server_info *server)
{
    ...
    if(NULL == pxCreatedTask){
        ret = xTaskCreate(upgrade_task, "upgrade_task", 512, server, 5, pxCreatedTask);//1024, 890 left
